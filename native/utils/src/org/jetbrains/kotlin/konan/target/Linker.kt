...
class AndroidLinker(targetProperties: AndroidConfigurables)
    ...
    override fun LinkerArguments.finalLinkCommands(): List<Command> {
        ...
        if (kind == LinkerOutputKind.STATIC_LIBRARY || kind == LinkerOutputKind.TAIHE_STATIC)
        ...
        return listOf(Command(clang).apply {
            ...
            when (kind) {
                ...
                LinkerOutputKind.TAIHE_DYNAMIC -> +listOf("-fPIC", "-shared")
                LinkerOutputKind.TAIHE_STATIC -> {}
...
class OhosLinker(targetProperties: OhosConfigurables)
    ...
    private val clangTarget = targetProperties.targetTriple.toString();
    private val clang = "$absoluteTargetToolchain/bin/${clangTarget}-clang++"
...
class GccBasedLinker(targetProperties: GccConfigurables)
    ...
    override fun LinkerArguments.finalLinkCommands(): List<Command> {
        ...
        val dynamic = kind == LinkerOutputKind.DYNAMIC_LIBRARY || kind == LinkerOutputKind.TAIHE_DYNAMIC
